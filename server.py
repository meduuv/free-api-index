...
REMOTE_SOURCES = [
    {"name": "public-api-lists", "url": "https://raw.githubusercontent.com/public-api-lists/public-api-lists/master/api/all.json", "format": "json"},
    {"name": "public-apis-live", "url": "https://raw.githubusercontent.com/Manavarya09/public-apis-live/main/data/apis.json", "format": "json"}
]
...
def canonical_url(value):
    value = normalize(value).lower().rstrip("/")
...
def normalize_item(item, source):
    ...
    description = normalize(item.get("description") or item.get("Description"))
    if not name or not url or not url.startswith(("http://", "https://")):
        return None
    return {"name": name, "api_url": url, "description": description, "auth": normalize(item.get("auth") or item.get("Auth") or "unknown"), "https": normalize(item.get("https") or item.get("HTTPS") or "unknown"), "cors": normalize(item.get("cors") or item.get("CORS") or "unknown"), "category": normalize(item.get("category") or item.get("Category") or "Other"), "source": source}
...
def parse_markdown(text, source, category):
    ...
    for line in text.splitlines():
        ...
        if not matches:
            continue
        rows.append(normalize_item({"name": cells[0], "api_url": matches[0], "description": cells[1], "category": category}, source))
    return [x for x in rows if x]

def fetch_ultimate():
    items = []
    for category in ULTIMATE_CATEGORIES:
        url = f"{ULTIMATE_BASE}/{urllib.parse.quote(category)}/README.md"
        try:
            request = urllib.request.Request(url, headers={"User-Agent": "free-api-index-agent/1.0"})
            with urllib.request.urlopen(request, timeout=30) as response:
...
def score(item, query):
    ...
    text = " ".join([str(item.get("name","")), str(item.get("description","")), str(item.get("category",""))]).lower()
    value = 0
    for part in parts:
        if part in str(item.get("name","")).lower():
            value += 5
        elif part in str(item.get("category","")).lower():
            value += 3
        elif part in text:
            value += 1
        else:
    ...
    def do_GET(self):
        ...
        if parsed.path in {"/search", "/select"}:
            ...
            if parsed.path == "/select":
                payload["agent_note"] = "Choose using task fit, auth, HTTPS, CORS, source and documentation. Verify availability before use."
